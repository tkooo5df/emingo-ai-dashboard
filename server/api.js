...
// Don't block server startup if database connection fails
(async () => {
  try {
...
app.post('/api/ai/chat', async (req, res) => {
  try {
    ...
    // Get text from either direct 'text' field or from messages array
    let inputText = text;
    if (!inputText && messages && Array.isArray(messages)) {
    ...
    if (!inputText) {
      return res.status(400).json({ 
        ...
        message: 'Please provide text or messages in the request body'
      });
    }
    // New API endpoint: http://fi8.bot-hosting.net:20163/elos-gemina?text=...
    const apiUrl = `http://fi8.bot-hosting.net:20163/elos-gemina?text=${encodeURIComponent(inputText)}`;
    ...
    if (!response.ok) {
      ...
      console.error('❌ Elos Gemina API Error:', {
      ...
      return res.status(response.status).json({ 
        error: 'Elos Gemina API error',
        message: errorText || response.statusText,
        status: response.status
      });
    }
    
    // Elos Gemina API returns JSON with 'response' field
    const data = await response.json();
    const answer = data.response || data.text || data.message || JSON.stringify(data);
...
async function generateDailyTip(language = 'en') {
  ...
  try {
    // Use new Elos Gemina API
    const apiUrl = `http://fi8.bot-hosting.net:20163/elos-gemina?text=${encodeURIComponent(prompt)}`;
    const response = await fetch(apiUrl, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
    ...
    if (!response.ok) {
      throw new Error(`Elos Gemina API error: ${response.status}`);
    }
...
app.get('/api/admin/data', authenticateUser, isAdmin, async (req, res) => {
  try {
    ...
    usersResult.rows.forEach((u, idx) => {
    });
...
if (process.env.NODE_ENV === 'production') {
  ...
  // Express 5 compatible - use middleware instead of wildcard
  app.use((req, res, next) => {
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  if (JWT_SECRET && JWT_SECRET !== 'your-secret-key-change-in-production') {
    ...
    console.error('⚠️ JWT_SECRET not set or using default value');
  }
