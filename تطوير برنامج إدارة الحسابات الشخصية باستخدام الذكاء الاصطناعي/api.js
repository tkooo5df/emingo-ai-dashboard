...
const isLocalhost = poolConfig.host.includes('localhost') || poolConfig.host.includes('127.0.0.1');

if (!isLocalhost) {
  // Disabling SSL as the connection to the dedicated IP failed the SSL handshake
  poolConfig.ssl = false;
}

// Add max, idleTimeoutMillis, connectionTimeoutMillis
poolConfig.max = 20;
...
function isAdmin(req, res, next) {
  ...
  if (req.userId && req.userId === adminUserId) {
    console.log('✅ [ADMIN] Admin access granted:', req.userId);
    next();
  } else {
    console.log('❌ [ADMIN] Admin access denied:', req.userId);
...
// Test endpoint to list all users (for debugging - remove in production)
console.log('🔧 Registering route: GET /api/users/list');
app.get('/api/users/list', authenticateUser, isAdmin, async (req, res) => {
  try {
    console.log('📥 [USERS LIST] Fetching all users...');
    const usersResult = await pool.query(
      'SELECT id, email, name, avatar_url, created_at FROM users ORDER BY created_at DESC'
    );
    console.log(`✅ [USERS LIST] Found ${usersResult.rows.length} users`);
    res.json({ 
      success: true,
...
// Test endpoint (no auth required) to verify route registration
console.log('🔧 Registering route: GET /api/admin/test');
app.get('/api/admin/test', (req, res) => {
  console.log('✅ [ADMIN TEST] Route is registered and accessible!');
  res.json({ 
    success: true, 
    message: 'Admin route is registered',
    timestamp: new Date().toISOString()
  });
});

// Analysis endpoint
console.log('🔧 Registering route: GET /api/analyze');
app.get('/api/analyze', authenticateUser, async (req, res) => {
  try {
    const userId = req.userId;
    console.log(`📥 [ANALYZE] Request received for user: ${userId}`);
    ...
    const [
      ...
      pool.query('SELECT amount, category, date FROM income WHERE user_id = $1 ORDER BY date DESC', [userId]),
      pool.query('SELECT amount, category, date FROM expenses WHERE user_id = $1 ORDER BY date DESC', [userId]),
      pool.query('SELECT name, target, current, deadline FROM goals WHERE user_id = $1', [userId])
    ]);
    ...
    // 2. Prepare data for AI analysis (last 3 months)
    ...
    const aiPrompt = `
      You are an AI financial and life coach. Analyze the following user data for the last 3 months and provide a concise summary, key insights, and actionable advice.
      
      **User Data (Last 3 Months):**
      - Income: ${JSON.stringify(recentIncome)}
      - Expenses: ${JSON.stringify(recentExpenses)}
      - Goals: ${JSON.stringify(goals)}
      
      **Analysis Requirements:**
      1.  **Summary:** A brief overview of the user's financial health (e.g., net flow, spending habits).
      2.  **Key Insights:** Highlight the top 3 spending categories and any significant deviations from the norm.
      3.  **Actionable Advice:** Provide 3 specific, personalized tips to improve their finances or achieve their goals faster.
      4.  **Goal Progress:** Comment on the feasibility of their current goals based on their spending.
      
      Respond only with the analysis text, formatted in Markdown.
    `;

    // 3. Call AI service (using the existing chat endpoint logic)
    const aiResponse = await fetch('https://api.you.com/v1/agents/runs', {
      method: 'POST',
      headers: {
    ...
    const analysisText = aiData.answer || 'Error: Could not generate AI analysis.';
...
// AI endpoints
console.log('🔧 Registering route: GET /api/aiapp.get('/api/admin/data', authenticateUser, isAdmin, async (req, res) => {
  try {
    console.log('📥 [ADMIN] ========================================');
    console.log('📥 [ADMIN] Request received from:', req.userEmail);
    console.log('📥 [ADMIN] User ID:', req.userId);
    console.log('📥 [ADMIN] Fetching all statistics and users...');
    ...
    const [
      ...
      pool.query('SELECT COALESCE(SUM(amount), 0) as total FROM income'),
      ...
      pool.query('SELECT id, email, name, avatar_url, created_at, updated_at FROM users ORDER BY created_at DESC')
    ]);
    ...
    const totalProjects = parseInt(totalProjectsResult.rows[0].total);
    const totalGoals = parseInt(totalGoalsResult.rows[0].total);
    const users = usersResult.rows;

    console.log('✅ [ADMIN] All statistics and users fetched successfully.');
    console.log(`📊 [ADMIN] Total Users: ${totalUsers}`);
    console.log(`📊 [ADMIN] Total Income: ${totalIncome}`);
    console.log(`📊 [ADMIN] Total Expenses: ${totalExpenses}`);
    ...
    res.status(500).json({ error: 'Failed to fetch admin data', message: error.message });
  }
});e.error('❌ [ADMIN] Error details:', {
      message: error.message,
      code: error.code,
      stack: error.stack
    });
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('   GET    /api/users/list (Admin only - List all users)');
});

// Debug endpoint to view all user data
// NOTE: This route is registered AFTER app.listen() for debugging purposes
// In production, move this before app.listen()
console.log('🔧 Registering route: GET /api/debug/data (AFTER app.listen - for debugging)');
